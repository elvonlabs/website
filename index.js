...
const buildJoinMailtoBody = (form, areas) => {
  const lines = [
    `Name: ${form.name}`,
    `Email: ${form.email}`,
    `Role: ${form.role}`,
...
const JoinSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const body = encodeURIComponent(buildJoinMailtoBody(form, areas));
    window.location.href = `mailto:${JOIN_CONTACT_EMAIL}?subject=${subject}&body=${body}`;
    setStatus('success');
  };
